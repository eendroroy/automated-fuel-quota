    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between">
        <span className="text-sm font-bold text-gray-900 dark:text-white">
          ...
          <span className="font-normal text-gray-400 dark:text-gray-500">{t('dashboard.remaining')}</span>
        </span>
        <span className="text-xs text-gray-400 dark:text-gray-500">of {quota.limitLiters}L</span>
      </div>
    ...
    <Modal isOpen onClose={onClose} title={t('dashboard.qrModalTitle')}>
      <div className="space-y-4">
        <div className="flex items-center gap-3 bg-gray-50 dark:bg-gray-800 rounded-xl px-4 py-3">
          <div className="h-9 w-9 bg-brand-100 dark:bg-brand-900/30 rounded-lg flex items-center justify-center flex-shrink-0">
            <Car className="h-4 w-4 text-brand-600" />
          </div>
          <div className="flex-1">
            <p className="font-bold text-gray-900 dark:text-white font-mono">{vehicle.registrationNumber}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400">{vehicle.vehicleMake} · {vehicle.vehicleColor} · {vehicle.fuelType}</p>
          </div>
...
function VehicleSpotlightCard({ vehicle, onShowQr }: {
  ...
  const borderColor =
    vehicle.status === 'UNVERIFIED' ? 'border-red-200 dark:border-red-800' :
    quotaAlmost ? 'border-amber-300 dark:border-amber-700' : 'border-gray-200 dark:border-gray-700'

  return (
    <div className={`card border ${borderColor} flex flex-col gap-4 transition-shadow hover:shadow-md`}>
      {/* Header */}
      <div className="flex items-start gap-3">
        <div className="h-11 w-11 bg-brand-50 dark:bg-brand-900/30 rounded-xl flex items-center justify-center flex-shrink-0">
          <Car className="h-5 w-5 text-brand-600" />
        </div>
        <div className="flex-1 min-w-0">
          <p className="font-bold text-gray-900 dark:text-white font-mono text-base leading-tight truncate">
            {vehicle.registrationNumber}
          </p>
          <p className="text-sm text-gray-500 dark:text-gray-400 truncate">{vehicle.vehicleMake} · {vehicle.vehicleColor}</p>
        </div>
        <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
          <StatusBadge status={vehicle.status} />
          <span className="text-xs text-gray-400 bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-full px-2 py-0.5">
            {vehicle.fuelType}
          </span>
        </div>
      </div>
      ...
      {vehicle.status === 'UNVERIFIED' && (
        <div className="border-t border-gray-50 dark:border-gray-700 pt-3">
          <div className="flex items-start gap-2 text-xs text-red-700 dark:text-red-300 bg-red-50 dark:bg-red-900/20 rounded-lg px-3 py-2">
      ...
      {/* Actions */}
      <div className="flex gap-2 border-t border-gray-50 dark:border-gray-700 pt-3 mt-auto">
        {vehicle.status === 'VERIFIED' && (
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        <div className="stat-pill">
          ...
          <div>
            <p className="text-xl font-bold text-gray-900 dark:text-white">{vehicles.length}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400">{t('dashboard.totalVehicles')}</p>
          </div>
        </div>
        <div className="stat-pill">
          <div className="h-9 w-9 bg-emerald-50 dark:bg-emerald-900/30 rounded-lg flex items-center justify-center flex-shrink-0">
            <CheckCircle2 className="h-5 w-5 text-emerald-600" />
          </div>
          <div>
            <p className="text-xl font-bold text-gray-900 dark:text-white">{verifiedVehicles.length}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400">{t('dashboard.activeQuotas')}</p>
          </div>
        </div>
        <div className="stat-pill col-span-2 sm:col-span-1">
          <div className="h-9 w-9 bg-blue-50 dark:bg-blue-900/30 rounded-lg flex items-center justify-center flex-shrink-0">
            <Fuel className="h-5 w-5 text-blue-600" />
          </div>
          <div>
            <p className="text-xl font-bold text-gray-900 dark:text-white">{transactions.length}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400">{t('dashboard.recentTransactions')}</p>
          </div>
        </div>
      </div>
      ...
      <section>
        ...
        {vehicles.length === 0 ? (
          <div className="card text-center py-16 border border-dashed border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
            <Car className="h-12 w-12 mx-auto mb-3 text-gray-300 dark:text-gray-600" />
            <p className="font-semibold text-gray-600 dark:text-gray-400 text-lg">{t('dashboard.noVehiclesYet')}</p>
          ...
          <>
            ...
            {(hiddenCount > 0 || vehicles.filter((v) => v.status === 'DEREGISTERED').length > 0) && (
              <Link
                to="/vehicles"
                className="mt-4 flex items-center justify-between w-full bg-gray-50 dark:bg-gray-800 hover:bg-brand-50 dark:hover:bg-brand-900/20 border border-gray-200 dark:border-gray-700 hover:border-brand-200 rounded-xl px-5 py-3.5 transition-colors group"
              >
                <div className="flex items-center gap-3">
                  <div className="h-8 w-8 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg flex items-center justify-center group-hover:border-brand-200">
                    <Car className="h-4 w-4 text-gray-400 group-hover:text-brand-500" />
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 group-hover:text-brand-700">
                      {hiddenCount > 0
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-semibold text-gray-900 dark:text-white text-lg">{t('dashboard.recentActivity')}</h2>
          </div>
        ...
        <div className="card p-0 overflow-hidden">
          {transactions.length === 0 ? (
            ...
            <div className="divide-y divide-gray-50 dark:divide-gray-800">
              {transactions.map((tx) => (
                <div key={tx.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                  <div className="h-9 w-9 bg-brand-50 dark:bg-brand-900/30 rounded-xl flex items-center justify-center flex-shrink-0">
                    <Fuel className="h-4 w-4 text-brand-600" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-gray-900 dark:text-white text-sm truncate">{tx.stationName}</p>
                    <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                      <span className="text-xs text-gray-400">{formatDateTime(tx.transactionTimestamp)}</span>
                      {tx.registrationNumber && (
                        <span className="text-xs font-mono bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded px-1.5 py-0.5">
                          {tx.registrationNumber}
                        </span>
                      )}
                    </div>
                  ...
                  <div className="text-right flex-shrink-0">
                    <p className="font-bold text-gray-900 dark:text-white text-sm">−{formatLitres(tx.amountDispensedLiters)}</p>
                    <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{formatLitres(tx.remainingQuotaAfter)} {t('dashboard.left')}</p>
                  </div>
                </div>
              ))}
            </div>
