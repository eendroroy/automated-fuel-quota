...
export default function CustomerClaimsPage() {
  const [claimsData, setClaimsData] = useState<PagedResponse<VehicleClaim> | null>(null)
  ...
  const load = useCallback((pageNum: number = 0) => {
    setLoading(true)
    getMyClaims({ page: pageNum, size: CLAIMS_PER_PAGE })
      .then((data) => {
        setClaimsData(data)
        setPage(pageNum)
      })
  ...
  const totalElements = claimsData?.totalElements || 0
  const totalPages = claimsData?.totalPages || 0
  return (
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {totalElements} {totalElements === 1 ? 'claim' : 'claims'} submitted
          </p>
        </div>
        ...
        <div className="space-y-4">
          {totalPages > 1 && (
            <div className="text-sm text-gray-500">
              Showing {page * CLAIMS_PER_PAGE + 1}–{Math.min((page + 1) * CLAIMS_PER_PAGE, totalElements)} of {totalElements}
            </div>
          )}
